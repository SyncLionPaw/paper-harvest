...
version: "0.1.0"
description: 帮助用户记录和复习英语单词。当用户说"记住单词 xxx"、"记一下单词 xxx"、"save the word xxx"，或要求"考考我"、"抽查单词"、"复习单词"、"quiz me on my words"时使用。单词保存在 ~/.english-words-reminder/words.txt，每行一个词和它的中文释义，末尾数字记录用户没记住的次数。
---
...
每行一个词条，用 Tab 分隔三列：`单词 <Tab> 中文释义 <Tab> 没记住的次数`。最后一列永远是次数（初始为 0）；释义在添加时一并保存。次数带负号表示该词已被移除（软删除）：词条保留在文件中，但 `list`/`pick`/`count` 都不会再出现。

```
serendipity	意外发现珍奇事物的运气	2
ephemeral	短暂的；朝生暮死的	0
in the long run	从长远来看	-1
```
...
```bash
"$SKILL_DIR/scripts/words.sh" pick 3
```
...
"$SKILL_DIR/scripts/words.sh" count         # 显示总词数
```

用户问"我记了哪些单词"时使用。报告时优先强调没记住次数高的词，建议用户重点复习。
